package com.personal.project.explora.db;

public final class DatabaseConstants {

    public static final int DB_VERSION = 2;

    public static final String V1_DB_FILENAME = "episodes-table";
    public static final String V1_EPISODE_TITLE_COLUMN = "title";

    public static final String EPISODE_TABLE_NAME = "episodes_table";
    public static final String EPISODE_ID_COLUMN = "id";

    public static final String EPISODE_YEAR_COLUMN = "year";
    public static final String EPISODE_DESCRIPTION_COLUMN = "description";
    public static final String EPISODE_LINK_COLUMN = "link";
    public static final String EPISODE_DATE_PUBLISHED_COLUMN = "datePublished";
    public static final String EPISODE_DOWNLOAD_ID_COLUMN = "downloadId";
    public static final String EPISODE_LAST_POSITION_COLUMN = "lastPosition";
    public static final String EPISODE_DURATION_COLUMN = "duration";
    public static final String EPISODE_RECENT_COLUMN = "recent";

}